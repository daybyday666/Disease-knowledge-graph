...
本项目的构建流程如下：**数据爬取 -> 预处理 (BIO标注) -> 实体抽取 (NER) -> 关系抽取 (RE) -> 图谱存储 (Neo4j) -> 智能问答 (GraphRAG) -> 可视化展示**。
...
### 5. 智能问答 (GraphRAG)
本项目创新性地引入了 **GraphRAG (Graph Retrieval-Augmented Generation)** 技术，解决通用大模型在垂直医疗领域存在的"幻觉"问题：
*   **机制**: 当用户提出医疗问题时，系统首先在自建的 Neo4j 知识图谱中进行语义检索，匹配相关的疾病、症状、药物等实体及其多跳关系子图。
*   **增强**: 将检索到的结构化医学知识（三元组）转化为自然语言上下文，作为 Prompt 的一部分输入给大模型。
*   **效果**: 确保了回答的专业性与准确性，使生成的内容严格基于丁香医生数据集的事实依据，实现了可解释、可溯源的医疗智能问答。

### 6. 系统应用 (Web Application)
